...
procedure Unicode.Source_Code.Conversion_To_Plain_Text.Convert_Rust is
   ...
   Text       : Wide_Wide_String :=
                  Unicode.IO.Read_File
                    ("src/test.rs", Ada.Strings.UTF_Encoding.UTF_8,
                     Actual_Encoding,
                     Uses_BOM);
...
begin
                  ...
                  for I in Positive range Position .. Text'Last + 1 loop
                     if Is_Line_Last (I) then
                        ...
                        Append_Atom (Converter, Text (Position .. I),
                                    ...
                                    Allows_LRM_Before => False));
                        Position := I + 1;
                     elsif I /= Text'Last and then Text (I .. I + 1) = "*/" then
                        Append_Atom (Converter, Text (Position .. I - 1),
                                    Lookahead (I - 1),
                                    (Comment_Content,
                                    Allows_LRM_Before => True));
   ...
   begin
      Unicode.IO.Write_File (Plain_Text, "src/test.fixed.rs",
                             Actual_Encoding, Uses_BOM);
   end;
end Unicode.Source_Code.Conversion_To_Plain_Text.Convert_Rust;
